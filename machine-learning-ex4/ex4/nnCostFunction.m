...
%
%%首先我们对y进行处理，使其变成前向输入时的基本格式
Y=[];
temp=eye(num_labels);
for i=1:num_labels
    yTemp = find(y==i);
    Y(yTemp,:) = repmat(temp(i,:),size(yTemp,1),1);
end
...
J= -1* sum(cost(:))/m + lambda * ( sum(temp1(:))+ sum(temp2(:)))/(2*m) ;
...
for t = 1:m
   ...
   delta2 = delta2 + err_3 * a_2';
   delta1 = delta1 + err_2 * a_1';
end
